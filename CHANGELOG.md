...
- Client integration details: the [Sidang Isbat Corrections](/isbat-koreksi) page.
...
- **JavaScript SDK** — `mabims-hijri` npm package, offline-first. Bundles MABIMS 2024-2026 data, `today()`, `convert()`, `range()`, `month()`, `year()`, `events()`, `hilal.info()`. Zero dependencies, works in Node, browser, edge runtime.
- **SDK docs** — `/sdk` page (installation, quick start, error handling, framework examples) and `/sdk/reference` (full function reference). Bilingual ID+EN.
- **Landing page** — "JavaScript SDK" card added as the last feature card.
- **Quickstart** — yellow callout pointing to SDK below code examples.
