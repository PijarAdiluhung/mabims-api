...
- [x] Bare (panel-less) hires web cards: `?bare=true` on `/hilal/viz` + `/hilal/map` returns a 1440×1520 card (header + graphic + logo) for the website hero — `render_bare_chart` / `render_bare_map`
- [x] `/hilal/history` from the precomputed `api/data/hilal_index.json` (1444-08 → 1475-12, `api/scripts/generate_hilal_index.py`) so the website renders the whole timeline in one request
- [x] Public `/hilal` page (non-Starlight, landing vibe): today's Hijri date, countdown to the next deciding evening, featured map + sky chart, and a lazy history list with inline info
- [x] Image pack builder (`api/scripts/build_imagepack.py`): card + bare in one deterministic tar + `manifest.json` (`render_version` from sites + points + renderer sources)
- [ ] Upload the combined pack to the CDN and fold index + image regen into the yearly `regen-computed-table.yml` flow
