...
<TestRequest path="/api/v1/convert" label="Coba /convert" />

<TestRequest path="/api/v1/range" label="Coba /range" />
...
Untuk `calendar=hijri`, rentang dapat melampaui cakupan tabel publik — bulan Hijriah di luar tabel dilayani dari perhitungan Neo MABIMS (hingga 2100). Batas `start`/`end` tetap maks 45 hari.
