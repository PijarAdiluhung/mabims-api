...
- **Field `weekday` di semua tanggal:** `/today`, `/today/{date}`, `/convert` (termasuk `next`), `/range`, `/month`, dan `/year` sekarang menyertakan nama hari dalam bahasa Indonesia untuk hari sipil yang sama: `"weekday": "Ahad"`, `"Selasa"`, ..., `"Sabtu"`. Hari Minggu ditulis **Ahad** (sesuai konvensi Kemenag), bukan "Minggu". Nama hari dihitung dari tanggal Masehi pada hari yang sama, sehingga pandangan Hijriah dan Masehi atas satu tanggal selalu sepakat.
- **Deskripsi OpenAPI** untuk field baru melalui `schema.weekday` (bilingual).

### Fixed

- **Event computed di bulan Hijriah terakhir yang sebagian tertutup:** `/events` dengan `include=extra` kembali mengembalikan event seperti Isra Mi'raj untuk bulan Hijriah di mana tabel kurasi belum lengkap (mis. 1448-07). Sebelumnya event yang dihitung dari komputasi bisa hilang pada batas akhir tabel.
- **Ayyamul bidh Dhul Hijjah:** Jendela hari putih di Dhul Hijjah kini bergeser ke 14–16 (bukan 13–15) karena tanggal 13 adalah Hari Tasyrik yang termasuk dalam event terpisah.

### Notes

- Perubahan bersifat tambahan (semver minor, tidak breaking): field baru selalu ada di `output`/item tanpa menghapus atau mengubah tipe field lama. Klien yang tidak membutuhkan nama hari bisa mengabaikannya.
- Sampel dokumen di halaman endpoint, landing, dan README turut diperbarui; situs dokumentasi kini menggunakan `output.weekday` langsung dari API dengan fallback lokal untuk snapshot lama.
...
- **Parameter `include` di `GET /api/v1/events`:** Lima hari besar bawaan tetap dikembalikan seperti sebelumnya; kini tersedia opsi tambahan: `include=extra` menambahkan peringatan tier-2 (Isra Mi'raj 27 Rajab, Nuzulul Quran 17 Ramadan, Arafah 9 Dzulhijjah, Tasu'a 9 Muharam, Asyura 10 Muharam, Hari Tasyrik 11–13 Dzulhijjah), `include=ayyamul_bidh` menambahkan puasa hari putih (13–15 setiap bulan Hijriah, 14–16 di Dzulhijjah karena tanggal 13 adalah Hari Tasyrik, satu event rentang per bulan, 12 setahun), slug individual bisa dipilih satu per satu, dan `include=all` mengembalikan semuanya.
- **Field `date_range` pada item event:** Event multi-hari (`tasyrik`, `ayyamul_bidh`) membawa `hijri_start`, `hijri_end`, `gregorian_start`, `gregorian_end`; event satu hari bernilai `date_range: null`. Field ini opsional dan tidak breaking.
- **Gema `input.include`:** Respons menggema set include yang diminta (tersortir, `null` bila tanpa parameter) sehingga klien dapat memverifikasi.
- **400 `invalid_include`:** Token yang tidak dikenal (mis. `include=ayahsura`) ditolak dengan pesan yang menyebut nilai yang salah.

### Notes

- Untuk klien: panggilan lama tetap menghasilkan `count: 5` dengan envelope yang sama; hanya permintaan opt-in yang bentuknya berubah. Semua tanggal baru merupakan pemetaan Hijriah tetap; di dalam tabel kurasi menggunakan `source: mabims`, di luar tabel mengikuti tier computed/retro seperti hari bawaan.
...
- **ETag + 304 Not Modified di semua respons 200:** Setiap respons kini membawa header **ETag**; kirim `If-None-Match: <etag>` (perbandingan lemah, atau `*`) dan server menjawab **304 tanpa body** dengan `Cache-Control` dan ETag yang sama. Berlaku di semua endpoint cacheable (`/today`, `today/{date}`, `convert`, `range`, `month`, `year`, `events`, `/hilal/*`).
- **Kode error `invalid_bare` dan `invalid_download`:** Parameter `bare`/`download` di `/hilal/viz` dan `/hilal/map` kini divalidasi ketat; nilai selain boolean ditolak dengan 400.
- **`GET /api/v1/months`:** Endpoint statis baru yang mengembalikan 12 nama bulan Hijriah beserta nomornya. Tanpa parameter, aman di-cache (`max-age=86400`), dikecualikan dari rate limit. Cocok untuk mengisi dropdown atau label di UI.
...
- **Tidak ada lagi 422 mentah dari FastAPI:** Input pecahan yang salah kini menghasilkan 400 (`invalid_year`/`invalid_month`/`missing_parameter`), dan boolean rusak kini menghasilkan 400 `invalid_<nama>`; `retro`/`next`/`bare`/`download` menerima `true`/`false` (tidak peka huruf besar/kecil) dan `1`/`0`. Parameter yang hilang atau kosong dianggap `false`.
- **Seed komputasi kini menutup seluruh rentang yang didukung:** Kira-kira 1945-01-01 (bulan Hijriah mulai 1944-12-17) sampai 2100-01-01, sehingga tidak ada lagi komputasi lazy saat runtime. Sebelumnya seed hanya 1970 sampai ~2050.
- **Warning borderline diperketat:** Bulan Hijriah ditandai "mendekati ambang visibilitas Neo MABIMS" **hanya bila** malam ke-29-nya **lolos** kedua kriteria (alt ≥ 3,0°, elongasi ≥ 6,4°) dengan margin **< 0,25°**. Bulan yang ditolak (tidak ada titik lolos) tidak pernah mendapat warning ini; sekitar 3% bulan terhitung borderline.
...
- Untuk klien: tangani `Retry-After` saat 429 dan gunakan `If-None-Match` untuk menghemat bandwidth; fallback 304 membuat polling tetap murah.
...
- **`servers` di spec OpenAPI:** Spesifikasi menyatakan `https://api.mabims.dev` sebagai server, sehingga klien yang di-generate/interaktif selalu resolve ke origin live tanpa konfigurasi tambahan.
...
- **Deskripsi OpenAPI ditulis ulang dan diterjemahkan:** Ringkasan dan deskripsi tiap endpoint ditulis ulang untuk kejelasan, lalu dialihbahasakan ke Indonesia.
...
- Murni dokumentasi dan spesifikasi; tidak ada perubahan perilaku runtime API maupun kontrak respons.
...
- **`?next=true` di `GET /today`:** Selain tanggal Hijriah hari ini, juga mengembalikan tanggal Hijriah yang mulai berlaku saat maghrib petang ini (pemetaan hari sipil berikutnya) lewat objek `next` dengan `source`-nya sendiri. API tidak menghitung maghrib; klien yang memutuskan ganti tanggal menggunakan jam salat maghrib masing-masing. Zona waktu mengikuti perilaku `/today`.
- **Koreksi Sidang Isbat:** Bila Sidang Isbat menetapkan awal bulan berbeda dari kalender terbit Kemenag, kini ada jalur koreksi resmi: `/meta` memunculkan **`divergences[]`** (riwayat koreksi, terbaru dulu) dan **`table_version`** (token versi riwayat; `"none"` kalau belum pernah ada koreksi, berubah setiap kali koreksi diterapkan).
...
- Selama belum ada koreksi tercatat, tidak ada yang berubah: `divergences[]` kosong dan `table_version = "none"`. Semuanya berupa tambahan field dan warning, tanpa breaking change.
...
- **Kartu bare (/hilal/viz + /hilal/map `?bare=true`):** Menghilangkan panel kriteria dan mengembalikan kartu resolusi tinggi **1440×1520** (header + grafik + callout/verdict + logo) untuk digunakan sebagai gambar hero di web. Kartu penuh 720×1280 tidak berubah. Varian bare ikut dipra-render ke image pack CDN (satu tar dengan `viz/` + `map/` + `viz-bare/` + `map-bare/`).
...
- **`scripts/build_imagepack.py`:** Pembangun image pack satu perintah: render (atau `--skip-render`) lalu tulis tar deterministik + `manifest.json` dengan `render_version` dari daftar titik, display points, dan sumber renderer.
- **Halaman publik `/hilal`:** Tanggal Hijriah hari ini, hitung mundur malam penentuan, peta + grafik bare yang bisa di-zoom, dan daftar riwayat dengan panel info inline. Situs kini menggunakan `LandingLayout.astro` bersama untuk landing dan `/hilal`.
...
- **Kap render kartu PNG: Hijri 1444–1475:** `hilal_image_range` di `/meta` kini merupakan batas keras `/hilal/viz`+`/hilal/map`; di luar rentang itu endpoint menolak dengan `out_of_coverage` (kap data tanggal tetap `coverage.forward_ceil` = 2100). Kartu pra-render terpaket tetap 1444–1450; bulan lain di dalam kap dirender on-demand beberapa detik dari cache astronomi.
- **Kartu peta: tabel kriteria → indikator rentang (gauge):** Baris `ALT. BULAN`/`ELONGASI` di `/hilal/map` kini menampilkan rentang **min–maks seluruh 95 titik tampilan** sebagai bar: merah = titik di bawah batas MABIMS, hijau = titik yang lolos, jarum dengan label `min. MABIMS` di ambang 3,0°/6,4°. Skalanya adaptif per bulan (mengisi bar di bulan nyaman, menunjukkan seberapa tipis bulan batas). Chip LOLOS/GAGAL dan kolom MIN./STATUS dihapus.
- **Font kartu hilal: DejaVu → Selawik:** Semua teks kartu `viz` + `map` kini menggunakan **Selawik** (SIL OFL, metrik-identik dengan Segoe UI), dikemas di `api/app/hilal/assets/` sehingga render Windows/laptop dan server Linux identik.
...
- 95 titik peta merupakan set **tampilan** untuk menunjukkan cakupan; verdict bulan tetap berasal dari model 25 titik pengamatan dan tidak berubah (0 perbedaan panjang bulan pada 1970–2051).
...
- **`deciding_site`** di `/hilal/info`: Objek `{ name, lat, lon, elev_m, tz }` titik yang digambarkan; titik penentu bila terlihat, atau titik dengan margin terbaik bila tidak terpenuhi di manapun (selalu terisi).
...
- **Halaman baru:** `/terms`, `/privacy`, `/data-sources`, `/disclaimer` (bilingual ID+EN), digabung di bawah sidebar "Legal".
...
- **Wording fixes:** "official/resmi" → "data publik" di seluruh README, landing page, footer, hilal docs, endpoint docs, playground, quickstart, FAQ, migration. Bahasa pemerintahan diredam.
- **Schema descriptions:** Field `source`, `warnings`, `visible`, `alt_ok`, `elong_ok` kini punya deskripsi OpenAPI.
- **Disclaimer section** ditambahkan ke README dengan klausul non-afiliasi.
