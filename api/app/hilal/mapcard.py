...
# ───────────────────────── astronomy ─────────────────────────
def _compute(lat: np.ndarray, lon: np.ndarray, evening: date):
    eph = _eph()
...
def _render_map(lo, la, altg, elongg, pts, alt, elong, hero, bounds, px=(2160, 1722)):
...
def _render_card(map_img, hero, n_total, n_seen,
    ...
    d = ImageDraw.Draw(card)
    _txt(d, (40, 32), f"Visibilitas {vis_month} {vis_year}", font(42, True), pal["text"])
    ...
    crit = [
        ("ALT. BULAN", f"{alt_b:+.1f}\u00b0", "3.0\u00b0", alt_b >= ALT_MIN),
        ("ELONGASI", f"{elong_b:.1f}\u00b0", "6.4\u00b0", elong_b >= ELONG_MIN),
    ]
    ...
    yy += 14
    rows = [
        ("TITIK PENGAMAT", hero[0].split("/")[0].strip().split()[0]),
...
def map_png_bytes(*, evening: date, vis_month: str, vis_year: int, hijri_label: str,
    ...
    mimg = _render_map(lo, la, altg, elongg, pts, alt, elong, hero, bounds, px)
