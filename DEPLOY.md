...
`api.pixostudio.id` → API container, `mabims.pixostudio.id` → docs container (rename everywhere).
...
| `PUBLIC_API_BASE` | **build arg** for docs | `https://api.pixostudio.id` |
...
4. **Domains** tab:
   - `api.pixostudio.id` → service `api`, port `8000`
   - `mabims.pixostudio.id` → service `docs`, port `80`
...
1. Create pull zone `mabims-api`, origin `api.pixostudio.id` (origin URL with `https://`).
2. **Caching** tab:
...
3. Repeat for `mabims-docs` zone → origin `mabims.pixostudio.id`; here you *can* override cache time
...
```bash
curl -s https://api.pixostudio.id/healthz
curl -sI https://api.pixostudio.id/api/v1/today          # expect s-maxage=<seconds to midnight>
curl -s "https://api.pixostudio.id/api/v1/convert?date=2025-01-03&calendar=gregorian"
curl -si -H "Origin: https://any.site" https://api.pixostudio.id/api/v1/today | head -1   # 200, CORS echoed
```
...
- [ ] Repoint partner integrations to `https://api.pixostudio.id/api/v1/*`
